...
static void smc911x_reset(void)
{
	...
	printf(DRIVERNAME": reset line 549 \n");
	...
	if (reg_read(PMT_CTRL) & PMT_CTRL_READY) {
		...
		timeout = 10;
		printf(DRIVERNAME": reset line 557 \n");
	...
	printf(DRIVERNAME": reset line 566 \n");

	/* Disable interrupts */
	reg_write(INT_EN, 0);
	printf(DRIVERNAME": reset line 570 \n");


	reg_write(HW_CFG, HW_CFG_SRST);
	printf(DRIVERNAME": reset line 574 \n");

	timeout = 1000;
	while (timeout-- && reg_read(E2P_CMD) & E2P_CMD_EPC_BUSY){	
		printf(DRIVERNAME": timeout = %d \n",timeout);
		udelay(10);
	}
	...
	printf(DRIVERNAME": reset line 588 \n");
	/* Set to LED outputs */
	reg_write(GPIO_CFG, 0x70070000);
	printf(DRIVERNAME": reset line 591 \n");
}
...
int eth_init(bd_t *bd)
{
	...
	if (val != 0x87654321) {
		...
		printf(DRIVERNAME ": valid chip endian 0x%08lx\n", val);
	}
