...
int nand_write_opts(nand_info_t *meminfo, const nand_write_options_t *opts)
{
	...
	printf("meminfo->writesize=0x%x,opts->writeoob=0x%x,meminfo->oobsize=0x%x\n",meminfo->writesize,opts->writeoob,meminfo->oobsize);

	/* check, if file is pagealigned */
	if ((!opts->pad) && ((imglen % pagelen) != 0)) {
		printf("opts->pad=0x%x,imglen=0x%x,pagelen=0x%x\n",opts->pad,imglen,pagelen);
